...
## Day 3
### Hierarchy :
- used for repeating a module  or library 
- " Lexical re-entrance" -  used to jump between different context  
...
### Branches
- Compute when a branch (instructions "bxxx") is needed, and where to branch ( PC of the branch and the immediate value of the instr). 
The branch target PC value will update the PC when previous instruction is a branch instruction (signal ```$taken_br``` will triger this).  

Final test example:  ```*passed = |cpu/xreg[10]>>5$value == (1+2+3+4+5+6+7+8+9);``` - testbench will monitor the value from ```/xreg[10]``` from RF . The name of the value from that register is ```$value```. ```>>5``` used to log more waveforms after the result is done , not to stop suddenly. When value will be equal with proper sum the simulation will stop.
  
## Day 5 - Pipelined RISC-V CPU
TLV is making the solution easier compare to Verilog.
Usually this is needed when we want to speed up the logic , so we split up in more stages, so we run less logic in one cycle.
To understand the pipelining is useful to look at the design in a waterfall representation  . 
...
A solution to this problem is to compute each step evrey x clock cycles. For this a "valid" signal is need with "x" cycles cadence - done before with a conter - and can be coded with the ```>>x``` .
A good practice is to initialize the cpu so a ```$start``` signal is required , immediate cycle after reset deasserts. This will generate also the first ```$valid```.  
```
$valid = $reset ? '0 : 
         $start ? '1 :
         >>3$valid;
...
### Solutions to Pipeline Hazards
We need solutions to run back to back instructions and get better performance.
First the register file dependencies - issue exampled is that we do not have time write in the previous instruction and read in the same instruction (read after write hazard).
IN the example is possible to bypass the ALU result from previous instr to current intsr ALU input.  

![](Day5/5-4.PNG) slide Reg file Bypass 2x
